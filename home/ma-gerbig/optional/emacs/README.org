...
For updading the packages run =M-x straight-pull-package= to get the latest version of a given package (or =M-x straight-pull-all= to update everything), and then =M-x straight-freeze-versions= to persist the on-disk versions to your lockfile (=~/config/emacs/straight/versions/default.el= by default).
...
=[[https://github.com/emacscollective/no-littering][no-littering]= package sets out to fix this by changing the values of path variables to put configuration files in =no-littering-etc-directory= (defaulting to "etc/" under =user-emacs-directory=, thus usually "~/.config/emacs/etc/") and persistent data files in =no-littering-var-directory= (defaulting to "var/" under =user-emacs-directory=, thus usually "~/.emacs.d/var/"), and by using descriptive file names and subdirectories when appropriate.
...
Use =~/.cache= to cointain local data.
#+BEGIN_SRC emacs-lisp :tangle no
  (defconst ma/cache-directory
    (expand-file-name ".cache/")
    "Directory where all cache files should be saved")

  (defun ma/cache-concat (name)
    "Return the absolute path of NAME under `ma/cache-directory'."
    (let* ((directory (file-name-as-directory ma/cache-directory))
...
#+BEGIN_SRC emacs-lisp :tangle no 
  (with-eval-after-load 'request
    (setq-default request-storage-directory (ma/cache-concat "request/")))
  (with-eval-after-load 'tramp
    (setq-default tramp-persistency-file-name (ma/cache-concat "tramp.eld")))
  (with-eval-after-load 'url
    (setq-default url-configuration-directory (ma/cache-concat "url/")))
#+END_SRC
